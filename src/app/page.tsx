...
export default function Home() {
  ...
  useEffect(() => { setPrediction(''); setFeedback(''); }, [step]);
  function go(value: number) { setPlaying(false); setStep(value); }
  function apply() {
    try { const parsed = parseInput(raw, size); setNums(parsed.nums); setK(parsed.k); go(0); setError(''); setPrediction(''); setFeedback(''); }
  ...
  return <div className="app-shell">
    ...
    <div className="main-shell">
      ...
      <main id="lesson">
        ...
        {tab === 'visualize' && <>
          <div className="story-strip"><span className="story-strip-icon"><Camera size={22} /></span><div><strong>Think of a camera sliding across a landscape.</strong><p>Keep a frame of {k} values. Let one leave, welcome one in, and look for the highest average.</p></div><button onClick={() => changeTab('story')}>Read the story <ArrowRight size={15} /></button></div>
          <div className="studio-grid">
            <section className="panel visualization"><div className="panel-heading"><div><span className="live-dot" /><h2>The sliding window</h2></div><span className="muted">INTERACTIVE CANVAS</span></div>
              ...
              <div className="canvas"><div className="canvas-top"><span>nums <span className="mono">[{nums.length}]</span></span><span className="window-label">↔ &nbsp; window size = {k}</span></div><div className="array-scroll"><div className="array-track">{nums.map((num, i) => <div className="array-column" key={i}><span className="pointer">{i === frame.left && i <= frame.right ? 'L' : ''}{i === frame.right ? (i === frame.left ? ' · R' : 'R') : ''}</span><div className={`array-cell ${i >= frame.left && i <= frame.right ? 'in-window' : ''} ${frame.focus === i ? frame.kind === 'remove' ? 'outgoing' : 'incoming' : ''}`}>{num}</div><span className="array-index">{i}</span></div>)}</div></div><div className="legend"><span><i className="mint" />Current window</span><span><i className="orange" />Leaving</span><span><i className="purple" />Entering</span></div></div>
              <div className="metrics"><div><span>WINDOW SUM</span><strong data-testid="sum">{frame.sum}</strong></div><div><span>CURRENT AVERAGE</span><strong>{completeWindow ? format(frame.sum / k) : '—'}</strong></div><div className="best"><span><Target size={13} /> BEST AVERAGE</span><strong data-testid="best">{frame.best === null ? '—' : format(frame.best / k)}</strong></div></div>
              ...
              <div className="playback"><div className="timeline-row"><span>Step {step + 1} <span className="muted">of {frames.length}</span></span><input aria-label="Execution timeline" type="range" min="0" max={frames.length - 1} value={step} onChange={e => go(Number(e.target.value))} /></div><div className="controls"><button className="icon-button" aria-label="Reset" title="Reset" onClick={() => go(0)}><RotateCcw size={17} /></button><button className="secondary" disabled={step === 0} onClick={() => go(step - 1)}><ArrowLeft size={15} /> Back</button><button className="primary" disabled={step === frames.length - 1} onClick={() => setPlaying(!playing)}>{playing ? <Pause size={16} /> : <Play size={16} fill="currentColor" />}{playing ? 'Pause' : 'Play'}</button><button className="secondary" disabled={step === frames.length - 1} onClick={() => go(step + 1)}>Next <ArrowRight size={15} /></button><label className="speed"><select aria-label="Playback speed" value={speed} onChange={e => setSpeed(Number(e.target.value))}><option value={2400}>0.5× speed</option><option value={1200}>1× speed</option><option value={600}>2× speed</option></select></label></div></div>
            </section>
